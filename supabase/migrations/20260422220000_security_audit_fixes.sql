-- Security audit fixes: 4 CRITICAL + 3 HIGH issues

-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- FIX 1 (CRITICAL): Remove USING(true) prospect RLS policies;
--                   replace with SECURITY DEFINER token-gated RPCs
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
-- Returns exactly one prospect whose token matches and has not expired.
-- Called by anon/authed users via the magic-link form.
...
-- Patches allowed columns for the matching prospect.
-- Form-data columns are locked once submitted (until an update is requested).
...
BEGIN
  ...
  -- Form data is locked when submitted and no update has been requested
...
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- FIX 3 (CRITICAL): SECURITY DEFINER helper for design-locked
--                   authorization check
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
AS $$
BEGIN
  -- Admins and AEs can lock any client design
  IF public.has_role(p_user_id, 'admin') OR public.has_role(p_user_id, 'account_executive') THEN
    RETURN true;
  END IF;

  -- AMs can lock the designs of their assigned clients
  IF EXISTS (
...
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- FIX 5 (HIGH): Lock down activity log INSERT;
--               create SECURITY DEFINER RPC so actor identity
--               is always derived server-side from auth.uid()
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

-- Drop the open INSERT policy (allows any authenticated user to forge entries)
...
-- Staff-only direct INSERT (admin / AE only; AMs use the RPC below)
